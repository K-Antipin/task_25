...
const modalRegister = new bootstrap.Modal('#registerModal');
const modalView = new bootstrap.Modal('#viewModal');
const uploadsModal = new bootstrap.Modal('#uploadsModal');
const navBarNavigator = document.querySelector('#navBarNavigator');
const formLogin = document.forms.formLogin;
const formRegister = document.forms.formRegister;
const button = navBarNavigator.children[1].children[0];
const imgView = document.querySelector('#imgView');
const modalBodyMess = document.querySelector('#modalBodyMess');
const imgId = { id: null };
    ...
    send.then((data) => {
        ...
        if (data) {
            if (
                ...
                formLogin.password.classList.contains('is-invalid')
            ) {
                formLogin.login.classList.remove('is-invalid');
                formLogin.password.classList.remove('is-invalid');
                formLogin.children[0].childNodes[1].innerText = 'Введите логин';
                formLogin.children[0].childNodes[1].style.color = '';
                formLogin.children[1].childNodes[1].innerText =
                    'Введите пароль';
                formLogin.children[1].childNodes[1].style.color = '';
            }
            switch (data.error) {
                case 'Логин и пароль отсутствуют':
                    formLogin.login.classList.add('is-invalid');
                ...
                case 'Пользователь не найден':
                    formLogin.children[0].childNodes[1].innerText =
                        'Пользователь не найден';
                    formLogin.children[0].childNodes[1].style.color = 'red';
                    formLogin.login.classList.add('is-invalid');
                    break;
                case 'Введите пароль':
                    formLogin.password.classList.add('is-invalid');
                ...
                case 'Не верный пароль':
                    formLogin.children[1].childNodes[1].innerText =
                        'Не верный пароль';
                    formLogin.children[1].childNodes[1].style.color = 'red';
                    formLogin.password.classList.add('is-invalid');
                    break;
                default:
                    modalLogin.hide();
...
formRegister.addEventListener('submit', (event) => {
    ...
    let form = {
        ...
        passwordConfirm: formRegister.passwordConfirm.value,
    };
    let send = submit(form, '/admin/register');
    send.then((data) => {
        if (data) {
            if (
                formRegister.name.classList.contains('is-invalid') ||
                formRegister.password.classList.contains('is-invalid')
            ) {
                formRegister.name.classList.remove('is-invalid');
                formRegister.password.classList.remove('is-invalid');
                formRegister.children[0].childNodes[1].innerText =
                    'Введите логин';
                formRegister.children[0].childNodes[1].style.color = '';
                formRegister.children[1].childNodes[1].innerText =
                    'Введите пароль';
                formRegister.children[1].childNodes[1].style.color = '';
            }
            switch (data.error) {
                case `Почта ${form.email} уже зарегистрирована`:
                    formRegister.email.classList.add('is-invalid');
                    formRegister.children[1].childNodes[1].innerText = `Почта ${form.email} уже зарегистрирована`;
                    break;

                case 'Пароль и подтверждение не совпадают':
                    formRegister.password.classList.add('is-invalid');
                    formRegister.children[2].childNodes[1].innerText = data.error;
                    formRegister.children[2].childNodes[1].style.color = 'red';
                    formRegister.passwordConfirm.classList.add('is-invalid');
                    formRegister.children[3].childNodes[1].innerText = data.error;
                    formRegister.children[3].childNodes[1].style.color = 'red';
                    break;
...
async function submit(data, url) {
...
if (document.querySelector('#logOut')) {
    document.querySelector('#logOut').addEventListener('click', () => {
        let out = submit(false, '/admin/exit');
        out.then((data) => {
            if (!data.error) {
                console.log(data)
                location.reload();
...
if (imgView) {
    imgView.addEventListener('click', (event) => {
        event.preventDefault();
        if (modalBodyMess.childElementCount)
            modalBodyMess.removeChild(modalBodyMess.childNodes[1]);
        if (document.querySelector('.bascetView'))
            document.querySelector('#pictureHeader').firstChild.remove();
        let img = document.querySelector('#imgModalView');
        imgId.id = event.target.getAttribute('data-img-id');
        document.querySelector('#pictureAuthor').innerText =
            'Автор: ' + event.target.getAttribute('data-user-name');
        img.src = event.target.src;
        if (event.target.getAttribute('data-user-id') === getCookie('id')) {
            let imgHeader = document.createElement('img');
            imgHeader.classList.add('bascetView');
            imgHeader.id = 'bascetViewDel';
            imgHeader.src = '/assets/img/icon/basket.svg';
            document.querySelector('#pictureHeader').prepend(imgHeader);
            imgHeader.addEventListener('click', () => {
                let imgDelete = submit({imgDelId: imgId.id}, '/events/deleteFiles');
                imgDelete.then((data) => {
                    if (isFinite(data.mess.idImg) && isFinite(data.mess.idMess) && data.mess.delFile) {
                        location.reload();
                    } else {
                        swal('Неизвестная ошибка', {
                            icon: 'error',
        ...
        let imgModalView = submit({ imgId: imgId.id }, '/events/showMess');
        imgModalView.then((data) => {
            if (data.mess) {
                ...
                modalBodyMess.appendChild(ul);
                let messagesList = document.querySelector('#messagesList');
                messagesList.addEventListener('click', (event) => {
                    let deleteCommetns = submit(
                        {
                            messId: event.target.parentNode.parentNode.getAttribute(
                                'data-mess-id'
                            ),
                        },
                        '/events/deleteMess'
                    );
                    deleteCommetns.then((data) => {
                        console.log(data);
                    });
...
if (document.forms.formComments) {
    document.forms.formComments.addEventListener('submit', (event) => {
        event.preventDefault();
        let addComments = submit(
            {
                message: document.forms.formComments.textarea.value,
                userId: getCookie('id'),
                imgId: imgId.id,
            },
            '/events/createMess'
        );
        addComments.then((data) => {
            document.forms.formComments.textarea.value = '';
            modalBodyMess.removeChild(modalBodyMess.childNodes[1]);
            let imgModalView = submit({ imgId: imgId.id }, '/events/showMess');
            imgModalView.then((data) => {
                if (data.mess) {
...
document.forms.formUploads.addEventListener('submit', async (event) => {
    event.preventDefault();
    let formUploads = await fetch('/events/createFiles', {
        method: 'POST',
        body: new FormData(document.forms.formUploads),
    });
    if (formUploads.ok) {
        let result = await formUploads.json();
        if (result.mess) {
            location.reload();
        } else {
            uploadsModal.hide();
